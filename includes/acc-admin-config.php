...
function acc_add_settings_link( $links, $file ) {
	...
	if ( strpos( $file, 'code-embed.php' ) !== false ) {
		$settings_link = '<a href="admin.php?page=acc-options">' . __( 'Settings' ) . '</a>';
...
function acc_set_plugin_meta( $links, $file ) {
	...
	if ( strpos( $file, '[plugin file name].php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="[support url]">' . __( 'Support' ) . '</a>' ) );
...
function acc_menu() {
	...
	add_menu_page( 'Artiss Currency Converter Settings', 'Currency', 'edit_posts', 'acc-options', 'acc_options', plugins_url() . '/artiss-currency-converter/images/menu_icon.png' );

    // Add search sub-menu

    if ( $contextual_help == 'new' ) { global $acc_options_hook; }

	$acc_options_hook = add_submenu_page( 'acc-options', 'Artiss Currency Converter Options', 'Options', 'edit_posts', 'acc-options', 'acc_options' );
    ...
    $acc_rates_hook = add_submenu_page( 'acc-options', 'Artiss Currency Converter Rates', 'Rates', 'manage_options', 'acc-rates', 'acc_rates' );
    ...
    if ( function_exists( 'wp_readme_parser' ) ) {
        add_submenu_page( 'acc-options', 'Artiss Currency Converter README', 'README', 'edit_posts', 'acc-readme', 'acc_readme' );
    }

    // Add support sub-menu

    add_submenu_page( 'acc-options', 'Artiss Currency Converter Support', 'Support', 'edit_posts', 'acc-support', 'acc_support' );
...
function acc_add_options_help() {
    ...
    $screen -> add_help_tab( array( 'id' => 'acc-options-help-tab', 'title'	=> __( 'Help' ), 'content' => acc_options_help() ) );
}
...
function acc_add_rates_help() {
    ...
    $screen -> add_help_tab( array( 'id' => 'acc-rates-help-tab', 'title' => __( 'Help' ), 'content' => acc_rates_help() ) );
}
...
function acc_options_help() {
	...
	$help_text = '<p>' . __( "This screen allows you to set some default options - if, when using the shortcode or function call, you don't specify a particular parameter then the default from this screen will be used." ) . '</p>';
	$help_text .= '<p>' . __( 'You can also set caching times as well - this is to limit the regularity of updates to the exchange rates and currency codes. The former is updated hourly and the latter ad-hoc, so retrieving this information everytime is not required.' ) . '</p>';
...
function acc_rates_help() {
	...
	$help_text = '<p>' . __( 'Use this screen to view the current exchange rates along with all the currency codes. All exchange rates are in relation to the US Dollar.' ) . '</p>';
	$help_text .= '<p>' . __( 'You can also perform a currency conversion from this screen.' ) . '</p>';
	$help_text .= '<h4>' . __( 'This plugin, and all support, is supplied for free, but <a title="Donate" href="http://artiss.co.uk/donate" target="_blank">donations</a> are always welcome.' ) . '</h4>';
...
function acc_admin_print_footer_scripts() {
    ...
    $pointer_content = '<h3>' . __( 'Welcome to Artiss Currency Converter' ) . '</h3>';
    $pointer_content .= '<p style="font-style:italic;">' . __( 'Thank you for installing this plugin.' ) . '</p>';
    $pointer_content .= '<p>' . __( 'A new menu has been added to the sidebar. This will allow you to change the default options (recommended!) and see the current exchange rate information.' );
?>
<script>
jQuery(function () {

...
function acc_get_rates( $cache = 60 ) {
    ...
    if ( !$rates ) {
        ...
        if ( $options[ 'id' ] == '' ) {
            ...
            $rates = '#' . __( 'No App ID has been setup - please set this in the options screen', 'currency-converter' );

        } else {

            $file = acc_get_file( 'http://openexchangerates.org/latest.json?app_id=' . $options[ 'id' ] );

            if ( $file[ 'rc' ] != 0 ) {

                $rates = '#' . __( 'Could not fetch exchange rate information', 'currency-converter' );
                ...
                if ( $rates == '' ) {
                    ...
                    $rates = '#' . __( 'No exchange rate information returned', 'currency-converter' );
...
function acc_get_codes( $cache = 24 ) {
    ...
    if ( !$codes ) {
        ...
        $file = acc_get_file( 'http://openexchangerates.org/currencies.json?app_id=' . $options[ 'id' ] );
        ...
        if ( $options[ 'id' ] == '' ) {
            ...
            if ( $file[ 'rc' ] != 0 ) {
                ...
                $rates = '#' . __( 'Could not fetch currency code information', 'currency-converter' );
                ...
                if ( $codes == '' ) {
                    ...
                    $rates = '#' . __( 'No currency code information returned', 'currency-converter' );
...
function acc_check_cache( $cache_value, $min_value ) {

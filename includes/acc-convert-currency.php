...
function acc_convert_currency( $number = '', $from = '', $to = '', $dp = '', $template = '' ) {
    ...
    if ( $number == '' ) {
        ...
        $result = '#' . __( 'No number supplied for conversion' );
    ...
    if ( substr( $result, 0, 1 ) == '#' ) {
        if ( $suppress_errors ) {
            ...
            return '<span style="font-weight: bold; color: #f00;">' . __( 'Artiss Currency Converter Error: ' ) . substr( $result, 1 ) . '</span>';
        }
    } else {
...
function acc_perform_conversion( $number = '', $from = '', $to = '', $dp = '' ) {
    ...
    if ( is_array( $rates_array ) ) {
        ...
        if ( ( $from == '' ) or ( $to == '' ) ) {
            ...
            $error = '#' . __( 'Could not fetch one of the required exchange rates' );
